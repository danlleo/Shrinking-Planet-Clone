...
namespace UI.WorkingSceneUI
{
    public class UnitDisplaySingleUI : MonoBehaviour
    {
        ...
        private readonly float _maxTimeInSeconds = .5f;
        ...
        public void Initialize(Sprite unitDisplayImage, string unitDisplayNameText, string unitDisplayLevelText,
            UnitLevel unitLevel, UnitEconomy unitEconomy, string unitSOName)
        {
        ...
        private IEnumerator StartLevelProgressMoveBarInSeconds()
        {
            ...
            while (timer <= _maxTimeInSeconds)
            {
                ...
                _progressBarForeground.fillAmount = Mathf.Lerp(xpLeftOversNormalizedValue, currentXPnormalizedValue,
                    InterpolateUtils.EaseInCubic(normalizedTime));
